...
function ChatContainer({ selectedUser, setSelectedUser }) {
  ...
  return selectedUser ? (
    <div className='flex flex-col h-full overflow-scroll relative backdrop-blue-lg'>
      {/* Header part */}
      ...
      <div className='flex flex-col h-[calc(100% - 120px)] overflow-y-scroll p-3 pb-16'>
        {messagesDummyData.map((msg, index) => (
          <div key={index} className={`flex items-end gap-2 justify-end ${msg.senderId !== '680f50e4f10f3cd28382ecf9' && 'flex-row-reverse'}`}>
            {msg.image ? (
              <img src={msg.image} alt='image' className='max-w-[230px] border border-gray-700 rounded-lg ovreflow-hidden mb-8' />
            ) : (
        ...
        {/* ===========bottom area */}
        <div className='absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3 backdrop-blur-2xl rounded-2xl'>
          <div className='flex-1 flex items-center bg-gray-100/12 px-3 rounded-full '>
            <input type="text" placeholder='Send A message' className='flex-1 text-sm p-3 border-none outline-none rounded-lg
          ...
          <img src={assets.send_button } className='w-7 cursor-pointer' alt="Icon is here" />
      </div>
    </div>
  ) : (
